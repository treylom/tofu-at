...
현재 모델이 opus가 아니거나 1M이 아닌 경우:
  "tofu-at는 opus 1M 컨텍스트에서 최적 작동합니다.
   `ai pass` 또는 `ain pass`로 시작하면 자동으로 opus 1M이 적용됩니다.
...
CC 시작 시 `ai pass` 또는 `ain pass`로 실행하면 **자동으로 1M 컨텍스트**가 적용됩니다:
```
ai pass        # opus[1m] + --dangerously-skip-permissions (자동 1M)
ain pass       # 새 tmux 윈도우에서 opus[1m] (자동 1M)
```

> **`pass` = 자동 1M**: `ai pass` / `ain pass` 사용 시 별도 `1m` 인자 없이 자동으로 opus 1M이 적용됩니다.
> `ai` (pass 없이) 실행 시에는 기본 200K 컨텍스트입니다.
...
|--------|------|
| `ai` | opus 200K (일반 모드) |
| `ai pass` | opus 1M + skip-permissions (자동 1M) |
| `ain pass` | 새 tmux 윈도우에서 opus 1M + skip-permissions (자동 1M) |
| `ain pass [name]` | 지정 세션명으로 opus 1M (자동 1M) |

> `pass` 사용 시 별도 `1m` 인자 없이 자동으로 1M 컨텍스트가 적용됩니다.
...
**`ai pass` 또는 `ain pass`로 시작하면 Main(리드)은 자동으로 opus 1M이 적용됩니다.**
